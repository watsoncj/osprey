...
const unitPath = "/etc/systemd/system/osprey-agent.service"
...
func installService(serverURL, hostname string, interval, lookback time.Duration, apiKey string) error {
	...
	cmd := fmt.Sprintf("%s -server %s -hostname %s -interval %s -lookback %s", exe, serverURL, hostname, interval, lookback)
	if apiKey != "" {
		cmd += " -api-key " + apiKey
	}
